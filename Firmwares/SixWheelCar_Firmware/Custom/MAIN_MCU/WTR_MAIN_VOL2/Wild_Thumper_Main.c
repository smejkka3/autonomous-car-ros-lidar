...
//After test the I2C communication it generates a test report,
...
void RF_ReceivedDataHandler1 (unsigned char ControlByte, char *InformationBytes)
{
	...
	if (block == false)
	{
		...
		switch (ControlByte)
		{
			...
			case 55:
			WriteDataHandler_Custom();
			I2CTWI_transmit4Bytes(I2C_WT_ADR, WRITE_COMMAND, ControlByte, InformationBytes[0], InformationBytes[1]);
			break;
