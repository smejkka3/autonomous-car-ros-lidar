...
std::string PowerReadingRail::only_output_values(){
    return this->voltage.get_value() + ' ' + this->current.get_value();
}
...
std::string PowerReadingDevice::to_pure_output(void){
    std::string output;

    this->update();
    for(std::vector<PowerReadingRail>::iterator rail = this->rails.begin(); rail != this->rails.end(); rail++){
        output += rail->only_output_values() + '\n';
    }
...
void to_pure_output(std::vector<PowerReadingDevice> &devices){
    for(int i = 0; i < 1; i++){
        std::cout << devices[i].to_pure_output();
    }
}
